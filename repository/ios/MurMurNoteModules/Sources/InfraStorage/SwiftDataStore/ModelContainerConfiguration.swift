...
public enum ModelContainerConfiguration {
    ...
    private static func configureDataProtection() {
        configureSecureStoreDirectory()
        configureAudioDirectory()
    }

    /// SecureStore ディレクトリの保護設定
    /// - NSFileProtectionComplete を適用
    /// - iCloudバックアップから除外
    private static func configureSecureStoreDirectory() {
        ...
        excludeFromBackup(url: storeDir)
        setFileProtection(url: storeDir, protection: .complete)
    }

    /// Audio ディレクトリの保護設定
    /// - NSFileProtectionComplete を適用
    /// - iCloudバックアップから除外
    private static func configureAudioDirectory() {
        ...
        excludeFromBackup(url: audioDir)
        setFileProtection(url: audioDir, protection: .complete)
    }
    ...
    /// ファイル保護レベルの設定
    static func setFileProtection(url: URL, protection: FileProtectionType) {
