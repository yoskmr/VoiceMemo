...
flowchart TB
    ...
    MemoList -->|"💬 アイコンタップ"| AskMemory
    MemoDetail -->|"タグタップ"| Search
    Search -->|"結果タップ"| MemoDetail
    AskMemory -->|"参照きおくタップ"| MemoDetail
    MemoDetail -->|"戻る"| AskMemory
    ...
    style AskMemory fill:#E8956A,color:#fff
...
全文検索（REQ-006）および高度フィルタリング（REQ-019）。
...
### 6.6 きおくに聞く画面（REQ-031）

#### エントリポイント

きおく一覧画面のナビゲーションバー右側に 💬 アイコンを配置。
...
- アイコン: `bubble.left.and.text.bubble.right` (SF Symbols)
- Proユーザー: Push遷移でチャット画面へ
- Freeユーザー: ハーフシート（`.medium` detent）でPro案内表示
- Freeユーザーのアイコン右上にProバッジ（8pt 星マーク）
...
- サジェスチョンはタップで入力欄に挿入し自動送信
- サジェスチョンカード背景: `vmPrimaryLight` 10% opacity
- 角丸: 16pt
...
- ユーザー質問: 右寄せ、背景 `vmPrimaryLight` 20%、角丸16pt
- AI回答: 左寄せ、背景 `vmSurfaceVariant`、左ボーダー `vmPrimary` 3pt、角丸16pt
- 参照きおくカード: 背景 `vmSurface`、角丸12pt、タップでメモ詳細へPush遷移
- 参照きおくカード内: 日付 + タイトル + 感情バッジ + タグチップ + ▶ 矢印
...
- カーソル `|` は0.5秒間隔で点滅
- 「きおくを探しています...」はFTS5検索中に表示
- 入力エリアが「⏹ 生成を止める」に変化（タップでストリーミング中断）
...
- CTAボタン「つぶやきを録音する」: `vmPrimary` 背景、タップで録音タブへ遷移
- プログレスバー: 現在のメモ件数 / 3件

#### Freeユーザー向けアップグレード案内（ハーフシート）
...
- Detent: `.medium`（画面の半分程度）
- 「Proプランを見てみる」: 設定 > 課金管理画面へ遷移
- 「あとで」: シートを閉じてきおく一覧に戻る
...
| 操作 | 応答 | アニメーション |
|:-----|:-----|:-------------|
| 💬 アイコンタップ（Pro） | チャット画面へ遷移 | Push (350ms, spring) |
| 💬 アイコンタップ（Free） | ハーフシート表示 | Sheet .medium (400ms) |
| 質問送信 | 質問バブル表示 → AI回答開始 | バブル: フェードイン + スライドアップ (250ms) |
| ストリーミング中 | テキスト逐次表示 + カーソル点滅 | 即時追加、カーソル0.5s点滅 |
| 参照きおくカード表示 | テキスト内にカード挿入 | フェードイン (200ms) |
| 参照きおくタップ | メモ詳細画面へ遷移 | Push (350ms, spring) |
| 「生成を止める」タップ | ストリーミング中断、テキスト保持 | ボタンフェードアウト → 入力欄復帰 (250ms) |
| サジェスチョンタップ | 入力欄に挿入 → 自動送信 | 縮小フェードアウト → 質問バブル表示 |

#### AIの回答トーン

- 寄り添い型: 「〜のようです」「〜かもしれませんね」
- 日付付き引用: 「3月25日のきおくでは〜と話していました」
- 押しつけない: 「〜すべき」は使わず「〜を振り返ってみるのも良いかもしれません」

#### コピーライティング

| 要素 | テキスト |
|:-----|:---------|
| プレースホルダー（初回） | きおくに何を聞きますか？ |
| プレースホルダー（会話中） | 続けて聞いてみましょう |
| エンプティステート | もう少しきおくを貯めましょう |
| ストリーミング中 | きおくを探しています... |
| エラー: ネットワーク | ネットワーク接続が必要です。接続を確認してもう一度お試しください。 |
| エラー: API | うまく応答できませんでした。少し時間をおいてお試しください。 |
...
Soyokaは「温かみ」を設計原則としているため、エラーメッセージも冷たい警告ではなくユーザーに寄り添うトーンとする。
