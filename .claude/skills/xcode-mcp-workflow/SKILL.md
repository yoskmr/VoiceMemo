...
description: MurMurNoteプロジェクトでのXcode MCP連携ルールとビルド(build)・テスト(test)手順。ビルド、テスト実行、デバッグ(debug)、エラー確認(error/warning)、SwiftUI Preview、Xcode操作など開発ワークフロー全般で必ず参照すること。コード変更後のビルド確認やテスト実行時、xcodebuild や swift test を使おうとした場面でも自動的に使うこと。
---
