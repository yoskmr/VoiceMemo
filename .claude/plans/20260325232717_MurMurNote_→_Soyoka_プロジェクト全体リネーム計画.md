...
- **対象**: MurMurNote / murmurnote / io.murmurnote / com.murmurnote の全参照
...
| バンドルID prefix | io.murmurnote | **app.soyoka** |
| バンドルID | io.murmurnote.app | **app.soyoka** |
| テストバンドルID | io.murmurnote.MurMurNoteTests | **app.soyoka.SoyokaTests** |
| Logger subsystem | com.murmurnote | **app.soyoka** |
| SPMパッケージ | MurMurNoteModules | **SoyokaModules** |
| App struct | MurMurNoteApp | **SoyokaApp** |
| CFBundleDisplayName | つぶやき | **Soyoka** |
| WelcomeView | Text("MurMurNote") | **「そよか」(メイン) + 「Soyoka」(サブ)** |

**備考**: `soyoka.app` ドメインは DNS NXDOMAIN 確認済み（未登録・取得可能）。
...
### Phase 2: ビルド設定更新（Phase 1完了後、Phase 4/5と並列可）

**2-A: `repository/ios/project.yml`**（20+箇所）
- `name: MurMurNote` → `name: Soyoka`
- `bundleIdPrefix: io.murmurnote` → `bundleIdPrefix: app.soyoka`
- 全 `MurMurNoteModules` → `SoyokaModules`
- 全 `MurMurNoteApp` → `SoyokaApp`
- `MurMurNote:` (target) → `Soyoka:`
- `MurMurNoteTests:` → `SoyokaTests:`
- `PRODUCT_BUNDLE_IDENTIFIER: io.murmurnote.app` → `PRODUCT_BUNDLE_IDENTIFIER: app.soyoka`
- `INFOPLIST_FILE: MurMurNoteApp/Info.plist` → `SoyokaApp/Info.plist`
- `CFBundleDisplayName: つぶやき` → `CFBundleDisplayName: Soyoka`

**2-B: `repository/ios/SoyokaModules/Package.swift`**（1箇所）
- `name: "MurMurNoteModules"` → `name: "SoyokaModules"`

### Phase 3: Xcode プロジェクト再生成（Phase 2完了後）

```bash
cd repository/ios && xcodegen generate
```

project.pbxproj の43箇所は XcodeGen が自動再生成。

### Phase 4: Swift ソースコード更新（Phase 1完了後、Phase 2と並列可）

| ファイル（リネーム後パス） | 変更内容 |
|:--|:--|
| `SoyokaApp/SoyokaApp.swift:10` | `struct MurMurNoteApp` → `struct SoyokaApp` |
| `SoyokaApp/WelcomeView.swift:22-23` | `Text("MurMurNote")` → `Text("そよか")` + サブテキスト `Text("Soyoka")` |
| `SoyokaModules/Sources/Data/AIProcessingQueueLive.swift:8` | `com.murmurnote` → `app.soyoka` |
| `SoyokaModules/Sources/InfraLLM/OnDeviceLLMProvider.swift:8` | `com.murmurnote` → `app.soyoka` |
| `SoyokaModules/Sources/InfraStorage/SwiftDataStore/ModelContainerConfiguration.swift:5` | `com.murmurnote` → `app.soyoka` |
| `SoyokaModules/Sources/InfraStorage/FTS5/FTS5IndexManager.swift:6` | `com.murmurnote` → `app.soyoka` |
| `SoyokaModules/Sources/InfraNetwork/Auth/KeychainManager.swift:5` | `com.murmurnote` → `app.soyoka` |

### Phase 5: ドキュメント・設定更新（Phase 1完了後、Phase 2と並列可）

**5-A: `repository/.swiftlint.yml`**
- コメント `MurMurNote` → `Soyoka`
- パス `VoiceMemoApp` → `SoyokaApp`, `VoiceMemoModules` → `SoyokaModules`（旧名修正も兼ねる）

**5-B: `.claude/agents/` 4ファイル内容**（MurMurNote → Soyoka, murmurnote → soyoka）

**5-C: `.claude/skills/` 2ファイル内容**（MurMurNote → Soyoka, パス更新）

**5-D: `CLAUDE.md`**（MurMurNote → Soyoka, ディレクトリ構成図のパス更新）

**5-E: `docs/spec/` + `docs/tasks/`**（15+ファイル、33箇所）
- MurMurNote → Soyoka, io.murmurnote → app.soyoka
...
grep -ri "murmurnote" .claude/agents/ .claude/skills/ CLAUDE.md docs/
...
- xcodegen 失敗: project.yml のパス確認、ディレクトリ存在チェック
- SPM ビルド失敗: `rm -rf .build && swift package resolve`
