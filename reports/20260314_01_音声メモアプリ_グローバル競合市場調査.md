...
quadrantChart
    ...
    x-axis Personal --> Business
    y-axis Simple --> AI-Powered
    quadrant-1 "AI x Business"
    quadrant-2 "AI x Personal"
    quadrant-3 "Simple x Personal"
    quadrant-4 "Simple x Business"
    Otter.ai: [0.85, 0.80]
    Granola: [0.80, 0.85]
...
> 横軸: 個人利用 → ビジネス利用 / 縦軸: シンプル機能 → AI高機能
...
```mermaid
quadrantChart
    title User Pain Points (Frequency x Severity)
    x-axis Low Frequency --> High Frequency
    y-axis Low Severity --> High Severity
    quadrant-1 "Top Priority"
    quadrant-2 "Watch Closely"
    quadrant-3 "Monitor"
    quadrant-4 "Room to Improve"
    Speech Recognition: [0.9, 0.9]
    Recording Loss: [0.6, 0.95]
    Search/Organization: [0.85, 0.7]
    Privacy Concerns: [0.7, 0.85]
    High Pricing: [0.8, 0.65]
    Multi-language: [0.65, 0.75]
    No Offline: [0.55, 0.6]
    Hard to Edit: [0.7, 0.55]
```

> 横軸: 頻度（低→高） / 縦軸: 深刻度（低→高）
> Speech Recognition=音声認識精度, Recording Loss=録音消失, Search/Organization=検索・整理不足, Privacy Concerns=プライバシー懸念, High Pricing=料金の高さ, Multi-language=多言語対応, No Offline=オフライン非対応, Hard to Edit=編集のしにくさ
